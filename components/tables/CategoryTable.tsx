...
const tableDatas = [
  { name: "Category", updated_at: "2022-04-12T21:14:44.942Z" },
];
const CategoryTable: FC<props> = () => {
  ...
  const navigateCategorySetting = () => {
    ...
    router.push({
      ...
      query: { local_id: 2, type: "category" },
    });
  };
  const navigateAdd = () => {
    setPageData({
      link: { title: "Add Categpry" },
      openFor: "add",
    });
    router.push({
      pathname: `/knowledge-base/categories/${33}/${33}`,
      query: { local_id: 2, type: "category" },
    });
  };
  ...
  return (
    <div className={`w-full flex flex-col  items-start`}>
      ...
      >
        <div
          className={`grid col-span-2 md:grid-cols-3 lg:basis-1/2 basis-full gap-5 `}
        >
        ...
        <div className={`w-full col-span-2 lg:col-span-1 flex flex-row`}>
          <div className="md:col-span-2 flex flex-row w-full justify-end ">
            <button
              onClick={navigateCategorySetting}
